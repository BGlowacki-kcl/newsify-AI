...
// const exclusionList = ['i', 'they', 'want'].map(word => word.toLowerCase());

// const FILE_PATH_TITLES = path.join(process.cwd(), 'titles.txt');
// const FILE_PATH_ARTICLES = path.join(process.cwd(), 'news_articles.txt');

// let augmentedPrompt = "";
// const systemPrompt = `
//   Act like a journalist. Be very specific and speak in the scientific language. 
...
// const titles = fs.readFileSync(FILE_PATH_TITLES, 'utf-8')
//     .split('\n')
//     .map(title => title.trim()) 
//     .filter(title => title.length > 0);
// const articles = fs.readFileSync(FILE_PATH_ARTICLES, 'utf-8')
//     .split('\n')
...
//         if (titleWords.includes(word) && !exclusionList.includes(word)) {
//             matchCount++;
//         }
//     });
...
// function getAugmentedPrompt(userMessageWords: any[]): string {
//     const titleScores = titles.map(title => ({
//         title,
//         score: getMatchingScore(title, userMessageWords),
//     }));
//     const bestMatches = titleScores.sort((a, b) => b.score - a.score).slice(0, 4);
//     const selectedArticles = articles.filter(article =>
//         bestMatches.some(title => article.includes(title.title))
//     );
//     return `
//         ${systemPrompt}
//         ${selectedArticles}
...
// function getRandomArticle(articles: any[]): any {
//     const randomIndex = Math.floor(Math.random() * titles.length);
//     return articles.filter(article => article.includes(titles[randomIndex]));
// }
...
//   let userMessage = data[data.length - 1];
//   const userMessageWords = userMessage.toLowerCase().split(/\s+/);

//   // if (data.length == 2) {
//   //   augmentedPrompt = getAugmentedPrompt(userMessageWords);
//   // }

//   const completion = await openai.chat.completions.create({
//     model: 'meta-llama/llama-3.1-8b-instruct:free',
//     messages: [{ role: 'system', content: augmentedPrompt }, ...data],
//     stream: true,
//   });
